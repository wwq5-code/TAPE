...
def prepare_verification_dataset(er_clean_on_re_set, er_on_rem_with_tri_set, er_clean_set, er_with_trigger_set,
                                 remaining_grad_set, args):
...
def calculate_hs_p(loss2, optimizer_hessian, vibi_f_hessian):
...
def get_grad_dataset(model, dataset_loader, dataset_loader2, erasing_size):
    dim_z = 256
    temp_img = torch.empty(0, 1, 28, 28).float().to(args.device)
    ...
    count_index = 0
    count_loss = 0
    for (x, y), (x2, y2) in zip(dataset_loader, dataset_loader2):
        x, y = x.to(args.device), y.to(args.device)  # (B, C, H, W), (B, 10)
        x2, y2 = x2.to(args.device), y2.to(args.device)  # (B, C, H, W), (B, 10)
        ...
        logits_z, logits_y, x_hat, mu, logvar, x_m, x_inverse_m = model(x,
                                                                        mode='with_reconstruction')  # (B, C* h* w), (B, N, 10)
        logits_z2, logits_y2, x_hat2, mu2, logvar2, x_m2, x_inverse_m2 = model(x2,
                                                                        mode='with_reconstruction')  # (B, C* h* w), (B, N, 10)
        ...
        log_z = torch.mean(logits_z.log_softmax(dim=1))
        log_y = torch.mean(logits_y.log_softmax(dim=1))
        kl_loss = nn.KLDivLoss(reduction="batchmean", log_target=True)
        ...
        '''purpose is to make the unlearning item =0, and the learning item =0 '''
        # the update loss of
        loss = args.beta * KLD_mean - args.mse_rate * BCE -  H_p_q #-  (log_z + log_y)
        # influence approximation
        loss_app = args.beta * KLD_mean + args.mse_rate * BCE + H_p_q  # -  (log_z + log_y)
        H_p_q2 = loss_fn(logits_y2, y2)
        ...
        x2 = x2.view(x2.size(0), -1)
        x_inverse_m2 = x_inverse_m2.view(x_inverse_m2.size(0), -1)
        # x = torch.sigmoid(torch.relu(x))
        BCE2 = reconstruction_function(x_hat2, x2)  # mse loss for vae

        # Calculate the L2-norm
        l2_norm_bce2 = torch.norm(args.beta * KLD_mean2 + args.mse_rate * BCE2, p=2)

        l2_norm_hpq2 = torch.norm(args.beta * KLD_mean2 + H_p_q2, p=2)

        total_u_s2 = l2_norm_bce2 + l2_norm_hpq2
        bce_rate2 = l2_norm_bce2 / total_u_s2
        hpq_rate2 = l2_norm_hpq2 / total_u_s2

        loss2 = (args.beta * KLD_mean2 + args.mse_rate * BCE2) * bce_rate2 + hpq_rate2 * (args.beta * KLD_mean2 + H_p_q2)

        unlearning_item = loss.item()

        # print(unlearning_item)
        learning_item = loss2.item()
        # print(learning_item)
        ...
        loss3 = unl_rate * loss + self_s_rate * loss2
...
re_re_set, er_on_remaining_set = torch.utils.data.random_split(remaining_set,
                                                               [remaining_size - erasing_size, erasing_size])
...
# Concatenate datasets and create a new loader
poison_trainset = ConcatDataset([dataset1, erased_on_rem_with_tri, erasing_set, erased_set_with_tri])
...
for epoch in range(args.num_epochs):
    vib.train()
    ''' For hbu, we directly train the full trained data, and then use hbu to unlearn the erasing samples '''

    vib, clean_acc_list, mse_list = vib_train(dataloader_full, vib, loss_fn, reconstruction_function, args, epoch,
                                              clean_acc_list,
...
temp_grad, temp_img, total_loss_on_re = get_grad_dataset(copy.deepcopy(vib), dataloader_er_clean_on_re, dataloader_remain, erasing_size)
...
temp_grad, temp_img, total_loss_tri_on_re = get_grad_dataset(copy.deepcopy(vib), dataloader_on_re_with_trigger,dataloader_remain,
                                                             erasing_size)
...
temp_grad, temp_img, total_loss_er_cl = get_grad_dataset(copy.deepcopy(vib), dataloader_er_clean, dataloader_remain, erasing_size)
...
temp_grad, temp_img, total_loss_tri_er_cl = get_grad_dataset(copy.deepcopy(vib), dataloader_er_with_trigger,dataloader_remain,
                                                             erasing_size)
...
temp_grad, temp_img, total_loss_remain = get_grad_dataset(copy.deepcopy(vib), dataloader_remain, dataloader_remain, erasing_size)
...
# Concatenate datasets and create a new loader
er_trainset = ConcatDataset([erased_on_rem_with_tri, erasing_set, erased_set_with_tri, er_on_remaining_set])
...
start_time = time.time()
reconstructor_er_re = train_reconstructor(copy.deepcopy(vib_full_trained), er_clean_on_re_train_loader,
                                          reconstruction_function, args)

end_time = time.time()
running_time = end_time - start_time
print(f'reconstruction Training took {running_time} seconds')

reconstructor_er_on_rem = train_reconstructor(copy.deepcopy(vib_full_trained), er_on_rem_with_tri_train_loader,
                                              reconstruction_function, args)

reconstructor_clean = train_reconstructor(copy.deepcopy(vib_full_trained), er_clean_train_loader,
                                          reconstruction_function, args)

reconstructor_bac = train_reconstructor(copy.deepcopy(vib_full_trained), er_with_trigger_train_loader,
                                        reconstruction_function, args)

avg_mse_on_re, average_sim_on_re, temp_img = get_avg_recon_MSE(copy.deepcopy(reconstructor_er_re),
                                                               copy.deepcopy(vib_full_trained),
                                                               er_clean_on_re_train_loader, reconstruction_function,
                                                               "er_clean_on_re")
...
avg_mse_tri_on_er, average_sim_tri_on_er, temp_img = get_avg_recon_MSE(copy.deepcopy(reconstructor_er_on_rem),
                                                                       copy.deepcopy(vib_full_trained),
                                                                       er_on_rem_with_tri_train_loader,
                                                                       reconstruction_function, "er_with_tri_on_re")
...
avg_mse_er_cl, average_sim_er_cl, temp_img = get_avg_recon_MSE(copy.deepcopy(reconstructor_clean),
                                                               copy.deepcopy(vib_full_trained), er_clean_train_loader,
                                                               reconstruction_function, "er_clean")
...
avg_mse_tri_er_cl, average_sim_tri_er_cl, temp_img = get_avg_recon_MSE(copy.deepcopy(reconstructor_bac),
                                                                       copy.deepcopy(vib_full_trained),
                                                                       er_with_trigger_train_loader,
                                                                       reconstruction_function, "er_with_trigger")
...
avg_mse_remain, average_sim_remain, temp_img = get_avg_recon_MSE(copy.deepcopy(reconstructor_er_on_rem),
                                                                 copy.deepcopy(vib_full_trained),
                                                                 remaining_grad_train_loader, reconstruction_function,
                                                                 "er_with_trigger")
...
constructed_verify = ConcatDataset(
    [er_clean_on_re_veri_set, er_on_rem_with_tri_veri_set, er_clean_veri_set, er_with_trigger_veri_set,
     remaining_grad_veri_set])
...
acc = infer_linear_from_vib_acc(classifier_model, vib_full_trained, dataloader_on_re_with_trigger, args,
                                name='on_re_with_trigger')

acc = infer_linear_from_vib_acc(classifier_model, vib_full_trained, dataloader_er_clean, args, name='er_clean')

acc = infer_linear_from_vib_acc(classifier_model, vib_full_trained, dataloader_er_with_trigger, args,
                                name='er_with_trigger')
...
#
# # samples on remaining set with backdoor
# dataloader_on_re_with_trigger = DataLoader(erased_on_rem_with_tri, batch_size=1, shuffle=True)
#
#
# # clean erased samples not in the remaining dataset
# dataloader_er_clean = DataLoader(erasing_set, batch_size=1, shuffle=True)
#
# # samples with backdoor trigger, not in the remainig dataset.
# dataloader_er_with_trigger = DataLoader(erased_set_with_tri, batch_size=1, shuffle=True)
...
acc = infer_in_or_not(copy.deepcopy(vib_full_trained), copy.deepcopy(reconstructor_er_re),
                      copy.deepcopy(classifier_model), er_clean_on_re_train_loader)

acc = infer_in_or_not(copy.deepcopy(vib_full_trained), copy.deepcopy(reconstructor_er_on_rem),
                      copy.deepcopy(classifier_model), er_on_rem_with_tri_train_loader)

acc = infer_in_or_not(copy.deepcopy(vib_full_trained), copy.deepcopy(reconstructor_clean),
                      copy.deepcopy(classifier_model), er_clean_train_loader)

acc = infer_in_or_not(copy.deepcopy(vib_full_trained), copy.deepcopy(reconstructor_bac),
                      copy.deepcopy(classifier_model), er_with_trigger_train_loader)
