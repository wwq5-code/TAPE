...
def get_grad_dataset(model, dataset_loader, dataset_loader2, erasing_size):
    ...
    for (x, y), (x2, y2) in zip(dataset_loader, dataset_loader2):
        ...
        params_with_hs = calculate_hs_p(loss2, optimizer, model)
        ...
        i = 0
        for p_hs, p, (name, param) in zip(params_with_hs, model.parameters(),model.named_parameters()):
            i = i + 1
            # if i==1:
            #     continue
            # print(p_hs.hess)
            ...
            if p.grad != None:
                ...
                # print(denom)
                # temp_g = p.data
                # result_grad = p.data.addcdiv_(exp_avg, denom, value=1)
                ...
                if name == 'encoder.linear_head.bias':
                    # print(param)
                    temp_img = torch.cat([temp_img, image], dim=0)
                    t_grad = exp_avg / denom  # result_grad - temp_g #param.grad.view(1, -1)
